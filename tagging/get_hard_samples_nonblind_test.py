...
def load(submission_dir):
    ...
    for sub in Path(submission_dir).iterdir():
        ...
        for scp in sub.rglob("*.scp"):
            metric = scp.stem
            if metric not in METRICS or re.match(r".*\.\d+", metric):
                continue
        ...
        for txt in sub.rglob("RESULTS.txt"):
            with txt.open("r") as f:
                for line in f:
                    ...
                    if metric not in METRICS or metric.startswith((" ", "WER")):
                        continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--submission_dir",
        type=str,
        default="submissions_nonblind_test",
        help="Path to the root directory containing all submissions",
    )
    parser.add_argument(
        "--outfile",
        type=str,
        default="output_nonblind_test.scp",
